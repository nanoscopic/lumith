...
function _newscript($url,$onload) {
    var $n = document.createElement('script');
    $n.type = 'text/javascript';
    $n.src = $url;
    if( $onload ) {
        $n.onload = $onload;
        $n.onreadystatechange = $onload;
    }
    return $n;
}

function _newstyle($url,$onload) {
...
var PROTOCUT_SCRIPTS = {};
var PROTOCUT_SCRIPTS_BY_UID = {};

function _loadscript($url,$onload) {
    var info;
    if( PROTOCUT_SCRIPTS[ $url ] ) {
        info = PROTOCUT_SCRIPTS[ $url ];
        if( info.loaded ) $onload();
        else info.onload.push( $onload );
    }
    else {
        info = {
            onload: [ $onload ],
        ...
        var head = _getbytag('head')[0];
        var node = _newscript($url,_autoScriptLoaded.bind(0,info) );
        _append( head, node );
    }
}
function _loadscriptUid($url,$uid,$onload) {
    if( PROTOCUT_SCRIPTS[ $url ] ) {
        info = PROTOCUT_SCRIPTS[ $url ];
        if( info.loaded ) $onload();
        else info.onload.push( $onload );
        PROTOCUT_SCRIPTS_BY_UID[ $uid ] = info;
    }
    else {
        info = {
            onload: [ $onload ],
        ...
        PROTOCUT_SCRIPTS_BY_UID[ $uid ] = info;
        var head = _getbytag('head')[0];
        var node = _newscript($url,0);
        _append( head, node );
    }
}
function _autoScriptLoaded(info) {
    ...
    info.loaded = 1;
    PROTOCUT_SCRIPTS[ info.url ] = info;
    info.onload = [];
}
function _scriptLoaded($uid) {
    var info = PROTOCUT_SCRIPTS_BY_UID[ $uid ];
    var onload = info.onload;
    for( var i=0;i<onload.length;i++ ) {
        var aload = onload[i];
        aload();
...
var PROTOCUT_STYLES = {};
function _loadstyle($url,$onload) {
    if( PROTOCUT_STYLES[ $url ] ) {
        if($onload) $onload(1);
        return 0;
    }
    PROTOCUT_STYLES[ $url ] = 1;
    var head = _getbytag('head')[0];
    var node = _newstyle($url,$onload||0);
    _append( head, node );
    return 1;
}
