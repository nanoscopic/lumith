...
sub init_inst {
    ...
    var attr = tag.attr || {};
    var name = attr.name || 'test';
    <tpl in=direct out=xjr>
        <input type='button' ref='el' value='*{name}' />
    </tpl>
    ...
    var node = res.refs.el.node;
    node.onclick = this.onclick.bind( this );
    this.domNodes = [ res.node ];
}
...
sub init {
    $mod_dc.register_widget( 'jsButton', %{instanceClass}, 1 );
}